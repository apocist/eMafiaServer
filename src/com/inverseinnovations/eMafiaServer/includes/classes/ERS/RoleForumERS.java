...
public class RoleForumERS{
	private RoleForum role;
	public RoleForumERS(final RoleForum role){
	...
	public RoleForumERS getTarget1(){
		RoleForumERS theReturn = null;
		if(role.getTarget1() > 0){
			RoleForum theRole = role.Match.getPlayerRole(role.getTarget1());
			if(theRole != null){theReturn = theRole.getERSClass();}
		}
		return theReturn;
	}
	public RoleForumERS getTarget2(){
		RoleForumERS theReturn = null;
		if(role.getTarget2() > 0){
			RoleForum theRole = role.Match.getPlayerRole(role.getTarget2());
			if(theRole != null){theReturn = theRole.getERSClass();}
		}
		return theReturn;
	}
	public TeamForumERS getTeam(){
